...
/**
 * @功能 图文混排TextView，请使用{@link #setWrapText(CharSequence)}
 */
...
//		}
	...
	protected void onDraw(Canvas canvas)
	{
		...
		//只有一行时
		if(oneLineWidth != -1 && contentList.size() > 0)
		{
		...
		for (int i = 0; i < contentList.size(); i++)
		{
			...
			for (int j = 0; j < line.line.size(); j++)
			{
				...
				{
					canvas.drawText((String) ob, realDrawedWidth, height + line.height - paint.getFontMetrics().descent, paint);
     ...
     * @param cs
     */
	public void setWrapText(CharSequence cs)
	{
		if (cs == null) {
			cs = "";
        }
